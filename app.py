...
def source_slug(mode: str, hospital_number: int | None) -> str:
    if mode == "provided" and hospital_number is not None:
...
with st.sidebar:
    ...
    if mode == "provided":
        hospital_number = st.selectbox("Hospital", [1, 2, 3, 4, 5], index=0)
        st.caption("Uses the Hospital 1–5 files included with this project.")
    else:
        st.markdown("#### Files")
    ...
    with st.expander("Advanced settings", expanded=False):
        ...
        st.divider()
        st.markdown("**Reuse contract rules**")
        st.caption(
            "Upload a previously downloaded contract-rules JSON file to skip contract analysis."
        )

        rules_upload = st.file_uploader(
            "Upload contract rules JSON",
            type=["json"],
            key="contract_rules_json",
        ...
        if rules_upload is not None:
            try:
                ...
                st.success("Contract rules loaded")
...
with tab_workflow:
    ...
    with contract_col:
        with st.container(border=True, height=370):
            ...
            with reset_col:
                has_analysis = st.session_state.get("contract_spec") is not None
            ...
            spec = st.session_state.get("contract_spec")
